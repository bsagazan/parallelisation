/* TIE-51257 Parallelization Excercise 2018
...
void parallelPhysicsEngine(int deltaTime){
   ...
   const int physicsUpdatesInOneFrame = 48000;
...
void fixedInit(unsigned int seed){
   ...
   for(int i = 0; i < SATELITE_COUNT; ++i){
      ...
      color id = {.red = randomNumber(0.f, 0.5f) + 0.5f,
                  .green = randomNumber(0.f, 0.5f) + 0.0f,
                  .blue = randomNumber(0.f, 0.5f) + 0.0f};
    
      // Random position with margins to borders
      vector initialPosition = {.x = HORIZONTAL_CENTER - randomNumber(50, 300),
                              .y = VERTICAL_CENTER - randomNumber(50, 300) };
